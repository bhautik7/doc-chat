...
from app.services.s3_service import upload_file_to_s3, delete_file_from_s3, get_file_from_s3
...
def list_user_documents(db: Session, user_id: int) -> list[Document]:
    return list_owned(db, Document, user_id)

def delete_document(db: Session, user_id: int, document_id: int) -> bool:
    document = get_owned(db, Document, user_id, document_id)
    if not document:
        return False
